    ...
    <>
      ...
      {!isOpen && !hasDismissed && secondsRemaining > 0 && (
        <aside
          ...
          className="fixed bottom-3 right-3 sm:bottom-5 sm:right-5 z-30 flex items-center gap-2 sm:gap-3 px-3 sm:px-3.5 py-1.5 sm:py-2 rounded-full bg-white/95 border border-[#EAE3D2] shadow-xl backdrop-blur-md text-[#1C1A17] text-xs font-mono transition-all animate-in fade-in slide-in-from-bottom-2 duration-300 max-w-[calc(100vw-24px)]"
        >
          ...
          <span className="text-[#8C8275] whitespace-nowrap text-[11px] sm:text-xs">
            Guest session: <strong className="text-[#9E7A45]">{secondsRemaining}s</strong>
          </span>
          <button
            onClick={() => setIsOpen(true)}
            className="px-2.5 py-0.5 rounded-full bg-[#FAF6EE] hover:bg-[#F5EEDB] text-[#9E7A45] border border-[#EAE3D2] text-[10px] sm:text-[11px] font-semibold transition-colors cursor-pointer shrink-0"
          >
            Sign In Now
          </button>
          <button
            onClick={handleDismiss}
            aria-label="Dismiss guest session timer"
            title="Dismiss guest timer"
            className="p-1 rounded-full text-[#8C8275] hover:text-[#1C1A17] hover:bg-[#FAF6EE] transition-colors cursor-pointer shrink-0 ml-0.5"
          >
            <X className="w-3 h-3" />
          </button>
        </aside>
